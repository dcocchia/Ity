...
function valueToText(value, doc = documentOrThrow()) {
    value = normalizeValue(value);
    if (value === null || value === undefined || value === false)
        return "";
    if (Array.isArray(value))
        return value.map((item) => valueToText(item, doc)).join("");
    ...
    if (isTemplateResult(value))
        return materializeTemplate(value, doc).textContent || "";
...
function normalizeSinkName(name) {
    return String(name).toLowerCase();
}
function isInlineHandlerSink(name) {
    return /^on/i.test(name);
}
function resolveURLSinkName(name) {
    const lower = normalizeSinkName(name);
    if (URL_SINK_ATTRIBUTE_NAMES.has(lower))
        return lower;
    return URL_SINK_PROPERTY_ALIASES.get(lower) || null;
}
function isAllowedDataURL(name, value) {
    if (name === "src") {
...
function isUnsafeURLValue(name, value) {
...
function sanitizeURLSinkValue(name, value, sink) {
    if (!isUnsafeURLValue(name, value))
        return value;
...
function sanitizeHTMLSinkValue(name, value, sink) {
...
function sanitizeAttributeValue(name, value, sink) {
    const lower = normalizeSinkName(name);
...
function applyPropertyBinding(element, name, value) {
    ...
    if (HTML_SINK_PROPERTY_NAMES.has(lower)) {
        element[name] = (_a = sanitizeHTMLSinkValue(name, value, `.${name}`)) !== null && _a !== void 0 ? _a : "";
        ensureManagedBindingMeta(element).props.set(name, value);
        return;
    }
    const urlSink = resolveURLSinkName(lower);
    if (urlSink) {
        if (value === null || value === undefined || value === false) {
            ...
            element[name] = (_b = sanitizeURLSinkValue(urlSink, String(value), `.${name}`)) !== null && _b !== void 0 ? _b : "";
        }
        ensureManagedBindingMeta(element).props.set(name, value);
        return;
    }
...
function compileTemplateBindings(strings, doc) {
    var _a, _b;
    const bindings = [];
    let source = "";
    for (let i = 0; i < strings.length - 1; i += 1) {
        const part = (_a = strings[i]) !== null && _a !== void 0 ? _a : "";
    ...
    source += (_b = strings[strings.length - 1]) !== null && _b !== void 0 ? _b : "";
    const template = doc.createElement("template");
        ...
        if (binding.kind === "node") {
            ...
            const path = marker
                ...
                : textBinding
                    ...
                    : null;
            if (!path)
                throw new Error(`Ity failed to compile node binding ${binding.index}.`);
            return {
                ...binding,
                path,
                markerText: textBinding === null || textBinding === void 0 ? void 0 : textBinding.markerText
            };
        }
        const attrName = `data-ity-bind-${binding.index}`;
        const element = template.content.querySelector(`[${attrName}]`);
        const path = element ? captureNodePath(template.content, element) : null;
        if (!element || !path)
            throw new Error(`Ity failed to compile attribute binding ${binding.index}.`);
...
function getCompiledTemplate(strings, doc) {
...
function materializeTemplate(result, doc = documentOrThrow()) {
    if (result.strings.length === 0) {
        return valueToFragment(result.values, doc);
    }
    ...
    for (const target of targets) {
        ...
        if (element)
            applyElementBinding(element, target.binding, value);
    }
    return fragment;
}
    ...
    _html(content, position) {
        for (const node of this.nodes) {
            ...
            if (position === "replace") {
                ...
                insertFragment(node, fragment, position);
            }
        }
        return this;
    }
...
function insertFragment(target, fragment, position) {
    ...
    switch (position) {
        case "beforebegin":
            (_a = target.parentNode) === null || _a === void 0 ? void 0 : _a.insertBefore(fragment, target);
            break;
        case "afterbegin":
        ...
        case "beforeend":
            ...
            break;
        case "afterend":
            (_b = target.parentNode) === null || _b === void 0 ? void 0 : _b.insertBefore(fragment, target.nextSibling);
            break;
    }
}
